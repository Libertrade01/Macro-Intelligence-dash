...
function prepareV2MacroBriefing(briefing) {
  ...
  return {
    ...
    meta: {
      ...
      inputCount: payload?.input_count || briefing.source_count || 0,
      sourceCount: payload?.source_count || payload?.sources?.length || briefing.sources?.length || 0,
      sources: payload?.sources || briefing.sources || [],
    },
    call: roomView,
    roomView,
    revision: {
